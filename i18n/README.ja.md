...
- `setup.bat` / `setup.ps1` — 先に言語、次にセーブパス。コピーして初期化まで自動
- `i18n/` — 翻訳 README とセットアップ文言。リポジトリ直下は英語のみ
...
`setup.bat` が先に**言語**（`en`、`zh-CN`、`ja` など）を尋ね、次にセーブパスを尋ね、**このリポジトリをそこにコピーして初期化まで自動で終わらせます**。二回目のコマンドは不要です。
